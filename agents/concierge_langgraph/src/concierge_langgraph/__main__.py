"""Run the Concierge as an A2A server."""
...
from .executor import ConciergeExecutor


def build_application():
    card = agent_card()
    handler = DefaultRequestHandler(
        agent_executor=ConciergeExecutor(),
...
def main() -> None:
    ...
    log.info(
        "Concierge (LangGraph) on %s  [reasoning=%s]",
        CONCIERGE.base_url,
        "llm" if settings().uses_llm else "deterministic",
    )
    run(build_application(), CONCIERGE)
